...
{$R *.dfm}
// Процидура открытия формы. Расставляет шашки и находит адрес программы
procedure dofield;
 var n,i,j:Integer;
  begin
  	for j:=1 to 8 do
...
if ((i+j) mod 2 = 0) then field[i,j]:='0';
 ...
 field[8,8]:='b';
   ...
   // Возможно перенесу размещение шашек на кнопку "новая игра"

   // Процидура просчета хода из диной точки даного игрока
procedure move(l:Char;x,y:Integer);
 ...
 // Процидура, которая рисует то, что выдает move
procedure drawmove;
 var n,i,j:Integer;
 begin
 	 for i:=1 to 8 do
...
// Процидура, которая просчитывает, есть ли боевые ходы
...
// Процидура, которая рисует вывод процидуры файт
...
// Процидура проверяет, есть ли новый выбраный имаг
...
//ПРоцидура реализует ход, но не ударный
...
// ПРоцедура превращает нужные шашки в дамки
...
//  процедура очищает доску от лишних пометок
procedure clean;
var i,j,n:Integer;
begin
for i:=1 to 8 do
...
begin
   if (not Assigned(Form2)) then   // проверка наличия окна
